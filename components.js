...
function createModal() {
  const modalHtml = `
  <div id="custom-modal" class="hidden fixed inset-0 z-[100] items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 transform transition-all">
      <div class="p-6">
        ...
        <p id="modal-message" class="text-gray-600 text-center mb-6 leading-relaxed whitespace-pre-line"></p>
  ...
  <nav class="fixed top-0 left-0 right-0 z-50 bg-gradient-to-r from-blue-900 to-blue-950 shadow-xl border-b-4 border-blue-950">
    <div class="container mx-auto px-4 py-4 flex items-center justify-between">
      ...
      <div class="hidden md:flex space-x-1 items-center">
        ...
        <a href="check_dupe_combo.html" id="nav-checkdupe" class="px-4 py-2 text-white hover:bg-blue-500 hover:text-white rounded-lg transition duration-200 font-medium">Check Dupe</a>
        ...
        <div class="relative group ml-2">
          ...
          <div class="absolute left-0 mt-1 w-56 bg-white rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-10 border border-gray-200">
            <a href="combine_combo.html" id="nav-combine" class="block px-5 py-3 text-gray-700 hover:text-blue-600 hover:bg-blue-50 font-medium first:rounded-t-lg transition">Combine Combo</a>
            <a href="combine_recovery.html" id="nav-recovery" class="block px-5 py-3 text-gray-700 hover:text-blue-600 hover:bg-blue-50 font-medium transition">Combine Recovery</a>
            <a href="setting_v4_config.html" id="nav-setting" class="block px-5 py-3 text-gray-700 hover:text-blue-600 hover:bg-blue-50 font-medium last:rounded-b-lg transition">Setting V4</a>
          </div>
        </div>
      </div>
      <div class="md:hidden">
    ...
    <div id="mobile-menu" class="hidden md:hidden bg-blue-600 shadow-md border-t border-blue-500">
      ...
      <a href="check_dupe_combo.html" id="nav-checkdupe-mobile" class="block px-5 py-3 text-white hover:bg-blue-500 font-medium transition">Check Dupe</a>
      <div class="px-5 py-3 border-t border-blue-500 bg-blue-700">
        <p class="text-white font-bold mb-2 text-sm">More Tools</p>
        <a href="combine_combo.html" id="nav-combine-mobile" class="block px-3 py-2 text-blue-100 hover:text-white hover:bg-blue-500 rounded font-medium text-sm transition">Combine Combo</a>
        <a href="combine_recovery.html" id="nav-recovery-mobile" class="block px-3 py-2 text-blue-100 hover:text-white hover:bg-blue-500 rounded font-medium text-sm transition">Combine Recovery</a>
        <a href="setting_v4_config.html" id="nav-setting-mobile" class="block px-3 py-2 text-blue-100 hover:text-white hover:bg-blue-500 rounded font-medium text-sm transition">Setting V4</a>
      </div>
    </div>
  </nav>
  `;
...
function setActiveNav() {
  ...
  const desktopNavIds = ['nav-split', 'nav-getcombo', 'nav-getcomboby', 'nav-checkdupe'];
  const dropdownNavIds = ['nav-combine', 'nav-recovery', 'nav-setting'];
  const mobileNavIds = ['nav-split-mobile', 'nav-getcombo-mobile', 'nav-getcomboby-mobile', 'nav-checkdupe-mobile', 'nav-combine-mobile', 'nav-recovery-mobile', 'nav-setting-mobile'];
  ...
  const pageMap = {
    ...
    'check_dupe_combo.html': ['nav-checkdupe', 'nav-checkdupe-mobile'],
  };
  ...
  allNavIds.forEach(id => {
    ...
    el.classList.remove('text-white', 'text-blue-700', 'text-blue-900', 'font-semibold', 'bg-blue-500', 'bg-blue-100', 'bg-blue-50', 'shadow-sm');

    if (desktopNavIds.includes(id)) {
      el.classList.add('text-white', 'hover:bg-blue-500');
      return;
    }

    if (dropdownNavIds.includes(id)) {
      el.classList.add('text-gray-700', 'hover:text-blue-700', 'hover:bg-blue-50');
      return;
    }

    if (mobileNavIds.includes(id)) {
      el.classList.add('text-white', 'hover:bg-blue-500');
    }
  });
  ...
  navIds.forEach(id => {
    ...
    if (desktopNavIds.includes(id) || mobileNavIds.includes(id)) {
      el.classList.add('text-white', 'font-semibold', 'bg-blue-500');
      return;
    }

    if (dropdownNavIds.includes(id)) {
      el.classList.add('text-blue-900', 'font-semibold', 'bg-blue-100');
    }
